...
export function nextBillTableField(
  ...
  if (line.manualEntry) {
    if (line.manualEntryOpen) {
      return nextStackedManualEntryField(current as ManualBillGridField, line, rateSlab)
    }
    return nextManualEntryField(current as ManualBillGridField, line, rateSlab)
  }
